...
__device__ __forceinline__ void warp_reduce(volatile float *input, unsigned tx) {
    // int x = input[tx];
    
    // x += input[tx + 32];   __syncwarp();
    // input[tx] = x;         __syncwarp();
    // x += input[tx + 16];   __syncwarp();
    // input[tx] = x;         __syncwarp();
    // x += input[tx + 8];    __syncwarp();
    // input[tx] = x;         __syncwarp();
    // x += input[tx + 4];    __syncwarp();
    // input[tx] = x;         __syncwarp();
    // x += input[tx + 2];    __syncwarp();
    // input[tx] = x;         __syncwarp();
...
__global__ void reduce5(float *input, float *output) {
    ...
    if (tx < 32) {
        warp_reduce<BLOCKDIM>(input_s, tx);
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < nWarmup + nIters; i++) {
        ...
        reduce5<BLOCKDIM, COARSE_FACTOR><<<gridDim, blockDim, sizeof(float) * BLOCKDIM>>>(input_d, output_d);
