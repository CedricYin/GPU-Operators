...
__global__ void transpose5_blockcoarse(float *input, float *output, size_t rows, size_t cols) {
    int ty = threadIdx.y;
    int tx = threadIdx.x;
    ...
    __shared__ float tile[TILEDIM * TILE_SCALING][TILEDIM + 1];  // bigger tile

    if (igy_start + ty < rows && igx_start + tx < cols) {
        #pragma unroll
        for (int i = 0; i < TILE_SCALING; i++) {
            tile[ty + i * TILEDIM][tx] = input[(igy_start + ty + i * TILEDIM) * cols + (igx_start + tx)];
        }
        __syncthreads();
        #pragma unroll
        for (int i = 0; i < TILE_SCALING; i++) {
            output[(ogy_start + ty) * rows + (ogx_start + tx + i * TILEDIM)] = tile[tx + i * TILEDIM][ty];
        }
    }
}
...
int main() {
    ...
    for (int i = 0; i < nWarmup + nIter; i++) {
        ...
        transpose5_blockcoarse<TILEDIM, TILE_SCALING><<<gridDim, blockDim>>>(a_d, b_d, ROWS, COLS);
