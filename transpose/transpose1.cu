...
__global__ void transpose0_baseline(const float *a, float *b, const size_t rows, const size_t cols) {
    const unsigned col = blockDim.x * blockIdx.x + threadIdx.x;
    const unsigned row = blockDim.y * blockIdx.y + threadIdx.y;
    const unsigned idx_b = row * cols + col;
    const unsigned idx_a = col * rows + row;
