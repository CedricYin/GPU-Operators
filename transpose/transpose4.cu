...
__global__ void transpose4_bankconflic(float *input, float *output, size_t rows, size_t cols) {
    int ty = threadIdx.y;
    int tx = threadIdx.x;
    ...
    __shared__ float tile[TILEDIM][TILEDIM + 1];  // remove bank conflic
...
int main() {
    ...
    for (int i = 0; i < nWarmup + nIter; i++) {
        ...
        transpose4_bankconflic<TILEDIM><<<gridDim, blockDim>>>(a_d, b_d, ROWS, COLS);
