...
__global__ void sgemm_v4(float *__restrict__ A, float *__restrict__ B, float *C, 
                        int M, int N, int K,
    ...
    // double buffer
    __shared__ float As[2][TK][TM];  // 为了便于float4读取，这里As进行转置
    ...
    for (int phase = 0; phase < K; phase += TK) {
        ...
        for (int i = 0; i < TM; i += a_tile_stride) {
            ...
            // 下面四条存在bank conflict
            As[write_idx][a_tile_x][a_tile_y + i] = a_tmp[0];
            As[write_idx][a_tile_x + 1][a_tile_y + i] = a_tmp[1];
            As[write_idx][a_tile_x + 2][a_tile_y + i] = a_tmp[2];
            As[write_idx][a_tile_x + 3][a_tile_y + i] = a_tmp[3];
        }
        for (int i = 0; i < TK; i += b_tile_stride) {
            FETCH_FLOAT4(Bs[write_idx][b_tile_y + i][b_tile_x]) = FETCH_FLOAT4(B[OFFSET(b_tile_y + i, b_tile_x, N)]);
        }
        __syncthreads();
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < nIters + nWarmup; i++) {
        ...
        sgemm_v4<TM, TN, TK, RM, RN><<<gridDim, blockDim>>>(a_d, b_d, c_d, M, N, K, alpha, beta);
