...
__global__ void sgemm_v5(float *__restrict__ A, float *__restrict__ B, float *C, 
    ...
    // double buffer
    __shared__ float As[TK][TM + 4];  // 为了解决bank conflic以及对齐float4，直接padding四个单位
    ...
    for (int phase = 0; phase < K; phase += TK) {
        ...
        for (int k = 0; k < TK; k++) {
            ...
            FETCH_FLOAT4(Areg[0]) = FETCH_FLOAT4(As[k][y_st]);
            FETCH_FLOAT4(Areg[4]) = FETCH_FLOAT4(As[k][y_st + TM / 2]);  
            FETCH_FLOAT4(Breg[0]) = FETCH_FLOAT4(Bs[k][x_st]);
            FETCH_FLOAT4(Breg[4]) = FETCH_FLOAT4(Bs[k][x_st + TN / 2]);
